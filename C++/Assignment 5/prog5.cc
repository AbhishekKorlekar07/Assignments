 ...
 * At the beginning of the simulation, an additional ping pong is released from top of the box which falls on the mousetrap. That mouse trap springs and shoots
 * its two ping pong balls and eventually they are likely to set off more mousetraps untill the time there are no balls in the air. 
 * */

#include "prog5.h" //importing the header file in our program 


//This function print Total simulation time, max no of balls in air and percentage of sprung straps in the end
...
/*This function is used to print the grid after every 10 cycles. It is also called at the beginning and ending of the simulation.
...
/*
 *This routine is called to update vector ballCycles for a ball. It searches for the first value 0 in the vector and when it finds , it removes that element from 
 the vector and replaces it with simulation cycle values and thus gets out of the loop.
 * */
...
/* 
 * This routine checks for every value in the vector ballCycles. If the value is not 0 then it decrements the value and if the value is 0, it computes random number to
 * choose random mouse trap and checks if the mousetrap is sprung or not. If it is not sprung then it calls update_cycles routine and sets the chosen mouse trap
 * to boolean value false and if it is sprung then it removes the element 0 from the vector.
 * */
...
/* 
 *This routing sets the srand function to time(0) and also inserts boolean value true into a multidimensional vector of GRID size 25. It also displays the grid for the clock 0 and  inserts the value 1 into the vector ballCycles as the simulation cycle for the first ball.
 * */
